...
class PagosListAPIView(APIView):
    ...
    def get(self, request):
        ...
        """
        pagos = tabla_pagos.objects.all()  # Recupera todos los registros de la tabla de pagos
        serializer = TablaPagosSerializer(pagos, many=True)  # Serializa los datos para su retorno en formato JSON
        return Response(serializer.data)
    ...
    def post(self, request):
        ...
        @return Response Objeto HTTP Response:
            ...
            - Si los datos son inválidos, retorna los errores de validación y un código de estado 400 (BAD REQUEST).
        """
        serializer = TablaPagosSerializer(data=request.data)  # Deserializa los datos enviados en la solicitud
        if serializer.is_valid():  # Valida los datos recibidos
            serializer.save()  # Guarda el nuevo registro en la base de datos
            return Response(serializer.data, status=status.HTTP_201_CREATED)  # Retorna el registro creado
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)  # Retorna errores si la validación falla
...
class DatosBasicosCreateView(APIView):
    ...
    def get(self, request):
        ...
        """
        datosbasicos = Datos_basicos.objects.all()  # Recupera todos los registros de la tabla Datos_basicos
        serializer = DatosBasicosSerializer(datosbasicos, many=True)  # Serializa los datos para su retorno en formato JSON
        return Response(serializer.data)
    ...
    def post(self, request):
        ...
        tipo_usuario_exp = request.data.get('tipo_usuario')  # Obtiene el tipo de usuario desde el request
        ...
        if usuario:  # Si el usuario existe, se actualizan los datos
            ...
            if serializer.is_valid():
                ...
                if tipo_usuario_exp == 3:
                    ...
                    print(f"Profesor creado con cédula {usuario.cedula}, sin maestría asignada.")
            ...
            if serializer.is_valid():
                ...
                if tipo_usuario_exp == 3:
                    ...
                    print(f"Profesor creado con cédula {usuario_guardado.cedula}, sin maestría asignada.")
...
def admin_login(request):
    ...
    """
    import json
    data = json.loads(request.body)  # Obtiene los datos del cuerpo de la solicitud (JSON)
...
class BuscarCedulaEstView(APIView):
    ...
    def get(self, request):
        ...
        serializer = DatosBasicosSerializer(datosbasicos, many=True)  # Serializa los datos
    ...
    def post(self, request):
        ...
        estudiante = Datos_basicos.objects.filter(cedula=cedula_exp).first()  # Busca el estudiante por cédula
        
        if estudiante:  # Si el estudiante existe
            if estudiante.tipo_usuario == 2:  # Verifica si el tipo de usuario es estudiante
                serializer = DatosBasicosSerializer(estudiante)  # Serializa los datos del estudiante
                return Response(serializer.data, status=status.HTTP_200_OK)  # Retorna los datos del estudiante
            else:
...
class AlmacenarDatosEstView(APIView):
    ...
    def post(self, request):
        ...
        estado_estudiante = request.data.get('estado_estudiante')  # Estado del estudiante ("Activo" o "Inactivo")
        cod_maestria = request.data.get('cod_maestria')  # Código de la maestría asociada al estudiante
        ...
        if not datos_basicos:
            return Response({"message": "Estudiante no encontrado en Datos_basicos."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if cod_maestria and not datos_maestria_instance:
            return Response({"message": "Código de maestría no encontrado."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        estudiante, created = estudiante_datos.objects.update_or_create(
            ...
            defaults={  # Datos a actualizar o registrar
                ...
                'cod_maestria': datos_maestria_instance if cod_maestria else None  # Relaciona con la instancia de datos_maestria, si se proporciona
            }
        )

        if created:
            message = "Estudiante registrado con éxito."  # Mensaje si el estudiante es creado
        else:
        ...
        return Response({"message": message}, status=status.HTTP_200_OK)  # Usamos el código HTTP 200 OK
